...
export default function BidsPendingTab({ darkMode, deliveries, loading, seenBids, setSeenBids, onAcceptBid, onEdit, onDelete }: Props) {
  const { dm, card, sub, txt, sec } = getStyles(darkMode);
  const s = { dm, sub, txt, sec };

  return (
    <div>
      <h2 className={`text-2xl font-bold mb-4 ${txt}`}>Bids Pending Approval</h2>
      {loading ? <div className={`${card} text-center`}><p className={sub}>Loading...</p></div>
      : deliveries.length === 0
        ? <div className={`${card} text-center`}><AlertCircle className={`w-16 h-16 ${dm ? 'text-gray-600' : 'text-gray-300'} mx-auto mb-4`} /><p className={sub}>No bids pending approval.</p></div>
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">{req.bids.map((bid, i) => (
                <div key={bid.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div><p className="font-medium">{bid.amount.toLocaleString()} sats</p><p className="text-sm text-gray-500">{bid.reputation.toFixed(1)} {bid.completed_deliveries} deliveries</p></div>
