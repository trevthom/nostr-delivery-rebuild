    ...
    <div className={card}>
      ...
      <div className="space-y-6">
        ...
        {isPkg && <div><label className={`block text-sm font-medium ${dm ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Packages</label>
          {form.packages.map((pkg, i) => <div key={i} className={`${sec} rounded-lg p-4 mb-3`}><div className="flex items-center justify-between mb-3"><span className={`font-medium ${txt}`}>Package {i + 1}</span>{form.packages.length > 1 && <button onClick={() => setForm({ ...form, packages: form.packages.filter((_, j) => j !== i) })} className={`${dm ? 'text-red-400' : 'text-red-600'} text-sm`}>Remove</button>}</div>
            <div className="flex gap-2 mb-2">
              <select value={pkg.size} onChange={e => uPkg(i, { size: e.target.value as PkgSize })} className={`flex-1 px-4 py-3 border ${dm ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white'} rounded-lg focus:ring-2 focus:ring-orange-500`}><option value={PS.ENVELOPE}>Envelope</option><option value={PS.SMALL}>Small (1-5 lbs)</option><option value={PS.MEDIUM}>Medium (5-20 lbs)</option><option value={PS.LARGE}>Large (20-50 lbs)</option><option value={PS.EXTRA_LARGE}>Extra Large (50+ lbs)</option></select>
              <input type="text" value={pkg.description} onChange={e => uPkg(i, { description: e.target.value })} placeholder="Description (optional)" spellCheck={false} className={`flex-1 px-4 py-3 border ${dm ? 'border-gray-600 bg-gray-700 text-white placeholder-gray-400' : 'border-gray-300 bg-white'} rounded-lg focus:ring-2 focus:ring-orange-500`} />
            </div>
        ...
        {isPerson && <div><label className={`block text-sm font-medium ${dm ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Persons</label><div className={`${sec} rounded-lg p-4 space-y-4`}>
          <div className="flex gap-4"><div className="flex-1"><label className={`block text-sm ${dm ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Adults</label><input type="number" min="0" value={form.persons.adults} onChange={e => setForm({ ...form, persons: { ...form.persons, adults: parseInt(e.target.value) || 0 } })} className={`w-full px-4 py-3 border ${dm ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white'} rounded-lg focus:ring-2 focus:ring-orange-500`} /></div>
            <div className="flex-1"><label className={`block text-sm ${dm ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Children</label><input type="number" min="0" value={form.persons.children} onChange={e => setForm({ ...form, persons: { ...form.persons, children: parseInt(e.target.value) || 0 } })} className={`w-full px-4 py-3 border ${dm ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white'} rounded-lg focus:ring-2 focus:ring-orange-500`} /></div></div>
        ...
        <div><label className={`block text-sm font-medium ${dm ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Offer Amount (sats) *</label>
          <div className="flex items-start gap-4">
            <div className="flex items-center gap-2 flex-shrink-0">
              <Bitcoin className="w-5 h-5 text-orange-500" />
              <input type="number" value={form.offer} onChange={e => setForm({ ...form, offer: e.target.value })} placeholder="25000" className={`w-40 px-4 py-3 border ${dm ? 'border-gray-600 bg-gray-700 text-white placeholder-gray-400' : 'border-gray-300 bg-white'} rounded-lg focus:ring-2 focus:ring-orange-500`} />
            </div>
            <label className={`flex items-start gap-3 ${dm ? 'text-gray-300' : 'text-gray-700'} cursor-pointer flex-1 p-3 ${sec} rounded-lg`}>
              <input type="checkbox" checked={autoApprove} onChange={e => setAutoApprove(e.target.checked)} className="rounded w-5 h-5 mt-0.5 flex-shrink-0" />
