        ...
        : sorted.map(req => {
          ...
          <div key={req.id} className={`${card} ${myBid ? (dm ? 'ring-2 ring-green-600' : 'ring-2 ring-green-400') : ''}`}>
            ...
            {!myBid ? <div className="flex gap-2">
              <button onClick={() => onPlaceBid(req.id, req.offer_amount)} disabled={loading} className="flex-1 bg-green-500 hover:bg-green-600 disabled:bg-gray-300 text-white font-medium py-3 rounded-lg">Accept {req.offer_amount.toLocaleString()} sats</button>
